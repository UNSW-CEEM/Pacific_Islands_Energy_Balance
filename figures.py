...
size_dict = {"app": 18, "report": 21}
font_size = size_dict[mode]
simple_template = dict(
    layout=go.Layout(
        title = dict(font=dict(family="Calibri", size=24),y=0.9),
        # title_font=,
        ...
        xaxis=dict(
            showgrid=False, gridcolor="grey", showline=True, linecolor=line_color,automargin = True
        ),
        # if mode=="report":
        # margin=dict(t=20)
    )
...
def imports_to_GDP(year):
    ...
    fig2.update_layout(title="Imported petroleum products per capita")
    # print(summary_df)
...
def import_export_figure(df_imp, df_exp, Interest_list, year):
    ...
    fig.update_layout(
        title="{}, Total Imports = {}, Total Exports = {} ($million)".format(
...
def Generate_Sankey(year, country):
    ...
    color_dicts = {
        ...
        "Electricity: Primary Production": "red",
        "Heat: Primary Production": "red",
        ...
        "Other Electricity & Heat": "red",
        "Other Electricity & Heat 3": "red",
    }
    ...
    fig.update_layout(
        height=900, font=dict(family="Calibri", size=22, color=font_color)
    )
    fig.update_layout(
...
def decarbonization_scenarios(
    ...
    demand_df["Battery_inst_cost"][
        demand_df["Year"] <= decarb_year
    ] = installment_comm_battery  # Million Dollar per year

    demand_df["RE_cumulative"][demand_df["Year"] == decarb_year] = demand_df["Demand"][
        demand_df["Year"] == decarb_year
    ]  # GWh
    demand_df["Geothermal_inst"][
        demand_df["Year"] == geothermal_completion_year
    ] = geothermal_MW  # MW
    demand_df["Geothermal_GWh"][
        demand_df["Year"] >= geothermal_completion_year
    ] = geothermal_GWh  # GWh
    demand_df["Geothermal_inst_cost"][
        demand_df["Year"] <= geothermal_completion_year
    ] = (geothermal_MW * geothermal_CAPEX * 1000000) / (
        geothermal_completion_year - 2022 + 1
    )  # $Dollar

    if geothermal_completion_year <= decarb_year:
        step = (
            demand_df["Demand"][demand_df["Year"] == decarb_year] - geothermal_GWh
        ) / (
            years + 1
        )  # GWh
        ...
        step = (demand_df["Demand"][demand_df["Year"] == decarb_year]) / (
            years + 1
        )  # GWh
    ...
    for i, row in demand_df.iterrows():
        if i > 0:
            demand_df.at[i, "RE_cumulative"] = (
                demand_df.at[i - 1, "RE_cumulative"] + step
            )  # GWh

    demand_df["RE_cumulative"][demand_df["Year"] > decarb_year] = (
        demand_df["Demand"] - demand_df["Geothermal_GWh"]
    )
    demand_df["RE_cumulative"][demand_df["RE_cumulative"] < 0] = 0

    demand_df["Annual_RE"] = (
        demand_df["RE_cumulative"].shift(-1) - demand_df["RE_cumulative"]
    )
    ...
    demand_df["wind_inst"] = (
        demand_df["Annual_RE"] * total_wind_share
    ) / Wind_pot  # MW

    demand_df["Small PV+B"] = (
        demand_df["PV_inst"] * small_PV_share
    ) / 0.7  # MW #30% less performance for rooftop
    ...
    demand_df["RE_inst_cost"] = (
        1000000
        * (
            demand_df["Small PV+B"] * rooftop_PV_cost
            + demand_df["Small PV+B"] * 2 * res_batt_cost
            + demand_df["Large PV"] * PV_cost
        )
        + 1000000
        * demand_df["wind_inst"]
        * (small_wind_share * WindBatt_cost + large_wind_share * Wind_cost)
        + demand_df["Geothermal_inst_cost"]
        + demand_df["Battery_inst_cost"] * 1000000
    ) / 1000000  # M$

    demand_df["non_RE_demand_TJ"] = (
        demand_df["Demand"] - demand_df["RE_cumulative"] - demand_df["Geothermal_GWh"]
    ) / 0.2777
    demand_df["non_RE_demand_TJ"][demand_df["non_RE_demand_TJ"] < 0] = 0
    demand_df["diesel_litre_dec"] = demand_df["non_RE_demand_TJ"] / (
        diesel_HHV * Efficiency
    )  # L
    demand_df["diesel_cost_dec"] = (
        demand_df["diesel_litre_dec"] * diesel_price / 1000000
    )  # $MM
    demand_df["diesel_litre_bs"] = (demand_df["Demand"] / 0.2777) / (
        diesel_HHV * Efficiency
    )
    demand_df["diesel_cost_bs"] = (
        demand_df["diesel_litre_bs"] * diesel_price / 1000000
    )  # $MM

    if Country == "New Caledonia":
        # coal price is 400 USD/Tonne
        demand_df["diesel_cost_dec"] = demand_df["diesel_cost_dec"] / 2
        demand_df["Coal_dec_tonne"] = (
            demand_df["non_RE_demand_TJ"] / 0.02931
        )  # 0.02931TJ = 1 tonne of coal
        ...
        demand_df["coal_bs_tonne"] = (
            demand_df["Demand"] / 0.2777
        ) / 0.02931  # 0.02931TJ = 1 tonne of coal
        demand_df["coal_bs_tonne"] = (
            demand_df["coal_bs_tonne"] / 2
        )  # 0.5 total demand is met by coal
        demand_df["coal_bs_mdollar"] = demand_df["coal_bs_tonne"] * 400 / 1000000

        demand_df["diesel_cost_bs"] = (
            demand_df["diesel_cost_bs"] + demand_df["coal_bs_mdollar"]
        )
        demand_df["diesel_cost_dec"] = (
            demand_df["diesel_cost_dec"] + demand_df["coal_dec_mdollar"]
        )

    demand_df["Diesel_cost_saving"] = (
        demand_df["diesel_cost_bs"] - demand_df["diesel_cost_dec"]
    )  # $MM
    demand_df["Net_saving"] = (
        demand_df["Diesel_cost_saving"] - demand_df["RE_inst_cost"]
    )  # $MM
    ...
    demand_df["Emission_dec_ton"] = (
        demand_df["Demand"] - demand_df["RE_cumulative"] - demand_df["Geothermal_GWh"]
    ) * diesel_emission_intensity
    demand_df["Emission_dec_ton"][demand_df["Emission_dec_ton"] < 0] = 0
    demand_df["Emission_red"] = (
        demand_df["Emission_bs_ton"] - demand_df["Emission_dec_ton"]
    )
    demand_df["Emission_red_cum"] = demand_df["Emission_red"].cumsum()
    # add carbon price
    demand_df["Emission_red_saving"] = (
        demand_df["Emission_red"] * emission_dollarpertonne / 1000000
    )  # million dolar
    demand_df["Diesel_import_reduction"] = (
        demand_df["diesel_litre_bs"] - demand_df["diesel_litre_dec"]
    ) / 1000000  # Million Litre
    ...
    for i, row in demand_df.iterrows():
        demand_df.at[i, "Net_saving_discounted"] = (
            demand_df.at[i, "Net_saving"]
            * ((1 + inflation_rate) / (1 + discount_rate)) ** i
        )
        demand_df.at[i, "Emission_red_saving_discounted"] = (
            demand_df.at[i, "Emission_red_saving"]
            * ((1 + inflation_rate) / (1 + discount_rate)) ** i
        )
    demand_df["Net_saving_cumsum"] = demand_df["Net_saving_discounted"].cumsum()
    demand_df["Net_saving_emission_cumsum"] = demand_df[
        "Emission_red_saving_discounted"
    ].cumsum()
    ...
    fig.add_trace(
        go.Bar(
            x=demand_df["Year"],
            y=demand_df["Net_saving_discounted"],
            name="Annual net saving",
            marker_color="forestgreen",
        ),
    )
    # fig.add_trace(
    ...
    fig2.update_yaxes(
        title_text="Cumulative Saving ($m)",
        showline=True,
        showgrid=False,
        linecolor=line_color,
        gridcolor=line_color,
    ...
    fig3.update_layout(
        title="Breakdown of annual RE installation for 100% RE in {}".format(
            decarb_year
        )
    )
    ...
    fig4.add_trace(
        go.Bar(
            x=demand_df["Year"],
            y=demand_df["Emission_red"],
            name="Annual emission reduction",
            marker_color="forestgreen",
        )
    )
    fig4.add_trace(
        go.Scatter(
            x=demand_df["Year"],
            y=demand_df["Emission_red_cum"],
            name="Cumulative emission reduction",
            marker_color="lightsalmon",
        ),
        secondary_y=True,
    )
    ...
    fig4.update_yaxes(
        title_text="Annual (t CO2-e)",
        showline=True,
        linecolor=line_color,
        gridcolor=line_color,
    )
    fig4.update_yaxes(
        title_text="Cumulative (t CO2-e)",
        secondary_y=True,
        showline=True,
        showgrid=False,
        linecolor=line_color,
    ...
    fig4.update_layout(
        title="Annaul CO2-e emission reduction by achieving 100% RE in {}".format(
            decarb_year
        )
    )
    # print(summary_df)
    fig4.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig5.add_trace(
        go.Bar(
            x=demand_df["Year"],
            y=demand_df["Diesel_import_reduction"],
            name="Annual",
            marker_color="forestgreen",
        )
    )
    fig5.add_trace(
        go.Scatter(
            x=demand_df["Year"],
            y=demand_df["Diesel_import_reduction"].cumsum(),
            name="Cumulative",
            marker_color="lightsalmon",
        ),
        secondary_y=True,
    ...
    fig5.add_trace(
        go.Scatter(
            x=demand_df["Year"],
            y=[oil_imports_2019] * len(demand_df["Year"]),
            name="Net oil products import in 2019",
            marker_color="red",
        ),
        secondary_y=False,
    )
    ...
    fig5.update_yaxes(
        title_text="Annual (million Litre)",
        showline=True,
        linecolor=line_color,
        gridcolor=line_color,
    )
    fig5.update_yaxes(
        title_text="Cumulative (million Litre)",
        secondary_y=True,
        showline=True,
        showgrid=False,
        linecolor=line_color,
    ...
    # print(summary_df)
    fig5.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
...
def rooftop_PV_plot(available_buildings, PV_size):
    ...
    fig.update_xaxes(showgrid=False, showline=True, linecolor=line_color)
    fig.update_layout(
        font=dict(family="Calibri", size=16, color=font_color), hovermode="x"
    )
    fig.update_layout(
        {"plot_bgcolor": "rgba(0,0,0,0)", "paper_bgcolor": "rgba(0,0,0,0)"},
        legend=dict(
    ...
    fig.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    fig.update_layout(title="Population and average households size")
    fig.update_xaxes(
        showline=True,
        linecolor=line_color,
    ...
    fig2.add_trace(
        go.Bar(
            x=rooftop_df["Country"],
            y=rooftop_df["avaialble_homes"],
            text=rooftop_df["avaialble_homes"],
            name="Number of homes available for rooftop PV",
            marker_color="forestgreen",
        ),
        secondary_y=False,
    )
    fig2.update_yaxes(
        title_text="Number of homes available for rooftop PV",
        secondary_y=False,
        showline=True,
        showgrid=True,
        linecolor=line_color,
    ...
    fig2.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    fig2.update_layout(title="Number of homes available for rooftop PV ")

    fig3 = make_subplots(specs=[[{"secondary_y": False}]])
    fig3.add_trace(
        go.Bar(
            x=rooftop_df["Country"],
            y=rooftop_df["Generation_GWh"],
            text=rooftop_df["Generation_GWh"],
            name="Potential rooftop PV generation (GWh)",
            marker_color="forestgreen",
        ),
    )
    fig3.update_yaxes(
        title_text="Rooftop PV generation (GWh/year)",
        showline=True,
        showgrid=True,
        linecolor=line_color,
        gridcolor=line_color,
    ...
    fig3.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    fig3.update_layout(title="Potential rooftop PV generation")

    fig4 = make_subplots(specs=[[{"secondary_y": False}]])
    fig4.add_trace(
        go.Bar(
            x=rooftop_df["Country"],
            y=rooftop_df["Capacity_MW"],
            text=rooftop_df["Capacity_MW"],
            name="Rooftop PV capacity",
            marker_color="forestgreen",
        ),
    )

    fig4.update_yaxes(
        title_text="Potential rooftop PV capacity (MW)",
        secondary_y=False,
        showline=True,
        showgrid=True,
        linecolor=line_color,
    ...
    fig4.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    fig4.update_layout(title="Potential rooftop PV capacity")
...
def UNstats_plots(year):
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=summary_df["marine_to_import"],
            name="Int. marine bunkers",
            marker_color="forestgreen",
        )
    )
    fig.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["aviation_to_import"],
            name="Int. aviation bunkers",
            marker_color="lightsalmon",
        )
    )
    fig.update_layout(  # width=1500,
    ...
    fig2.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["transformation_to_import"],
            name="Transformation",
            marker_color="forestgreen",
        )
    )
    fig2.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["transformation_losses_to_import"],
            name="Transformation losses",
            marker_color="lightsalmon",
        )
    )
    fig2.update_layout(
    ...
    fig3.update_layout(
        title="Breakdown of imported oil consumed for domestic transport in {}".format(
            year
        )
    )
    fig3.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig4.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["Oil imports"],
            name="Oil imports",
            text=summary_df["Oil imports"],
            marker_color="forestgreen",
        )
    )
    ...
    fig4.update_layout(title="Imported oil in {}".format(year))
    fig4.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig5.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["Total_demand"],
            name="Total demand",
            text=summary_df["Total_demand"],
            marker_color="forestgreen",
        )
    )
    ...
    fig5.update_layout(title="Total demand (excluding int transit) in {}".format(year))
    # print(summary_df)
    fig5.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig6.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["renewables_in_total"],
            name="Total energy from renewables",
            text=summary_df["renewables_in_total"],
            marker_color="forestgreen",
        )
    )
    ...
    # print(summary_df)
    fig6.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig7.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["renewable_electricity"],
            name="Total energy from renewables",
            text=summary_df["renewable_electricity"],
            marker_color="forestgreen",
        )
    )
    ...
    # print(summary_df)
    fig7.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig8.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["Renewables/Total_demand"],
            name="Total energy from renewables",
            text=summary_df["Renewables/Total_demand"],
            marker_color="forestgreen",
        )
    )
    ...
    fig8.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    fig8.update_traces(hovertemplate=None)

    fig_re_imp = go.Figure()
    fig_re_imp.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["Renewables/Total_imports"],
            name="Total energy from renewables",
            text=summary_df["Renewables/Total_imports"],
            marker_color="forestgreen",
        )
    )
    ...
    fig_re_imp.update_yaxes(
        title_text="% of total energy imports",
        showline=True,
        linecolor=line_color,
        gridcolor=line_color,
    )
    fig_re_imp.update_xaxes(
        showline=True,
        linecolor=line_color,
        title_text='<a href="http://unstats.un.org/unsd/energystats/pubs/balance"><sub>Source: Energy Balances, United Nations<sub></a>',
    )
    ...
    fig_re_imp.update_traces(
        marker_line_color=font_color, marker_line_width=1.5, opacity=1
    )
    ...
    fig9.add_trace(
        go.Bar(
            x=summary_df["Country"],
            y=summary_df["Renewables/capita"],
            name="Total energy from renewables",
            text=summary_df["Renewables/capita"],
            marker_color="forestgreen",
        )
    )
    ...
    fig9.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
...
def land_use_plot():
    ...
    world_average_demand = functions.fetch_all_countries_demand(
        2019, Unit="GWh", Use="Analysis"
    )[11].round(0)
    ...
    fig1.add_trace(
        go.Bar(
            ...
            name="% coastline for final demand",
            marker_color="black",
        )
    )
    fig1.add_trace(
        go.Scatter(
            ...
            name="% coastline for decarbonizing the electricity sector",
            marker_color="red",
            mode="markers",
        )
    )
    ...
    fig3.add_trace(
        go.Scatter(
            x=countries,
            y=PV_area_non_RE_per,
            name="% of land for decarbonizing the electricity sector",
            marker_color="red",
        )
    )
    fig3.add_trace(
        go.Scatter(
            x=countries,
            y=PV_area_final_demand_per,
            name="% of land for final demand",
            marker_color="black",
        )
    )
    ...
    fig3.update_layout(
        ...
        font=dict(family="Calibri", size=14, color=font_color),
        hovermode="x",
    )
    fig3.update_layout(
        {
    ...
    fig4.add_trace(
        go.Bar(
            x=countries,
            y=world_average_demand,
            text=world_average_demand,
            name="World average per capita demand",
            marker_color="green",
        )
    )
    fig4.update_layout(
    ...
    fig4.update_layout(title="Demand for decarbonization")
    fig4.update_traces(marker_line_color=font_color, marker_line_width=0, opacity=1)
    ...
    fig5.add_trace(
        go.Bar(
            x=countries,
            y=non_RE_demand_per_capita,
            text=non_RE_demand_per_capita,
            name="Decarbonizing the electricity sector",
            marker_color="red",
        )
    )
    fig5.add_trace(
        go.Bar(
            x=countries,
            y=final_demand_per_capita,
            text=final_demand_per_capita,
            name="Meeting the final demand",
            marker_color="darkgrey",
        )
    )
    ...
    fig5.update_layout(title="Demand per capita")
    fig5.update_traces(marker_line_color=font_color, marker_line_width=0, opacity=1)
...
def generation_mix_plot():
    ...
    fig.update_layout(title="Generation mix in 2019")
    # print(summary_df)
    fig.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
    ...
    fig2.update_layout(title="Installed capacity mix in 2020")
    # print(summary_df)
    fig2.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
...
def cross_country_sankey(df, from_, to_, normalization):
    ...
    fig.update_yaxes(
        title_text=Unit, showline=True, linecolor=line_color, gridcolor=line_color
    )
    fig.update_xaxes(
        showline=True,
        linecolor=line_color,
    ...
    fig.update_layout(title="From {} to {} ({})".format(from_, to_, tail))
    # print(summary_df)
    fig.update_traces(marker_line_color=font_color, marker_line_width=1.5, opacity=1)
...
def Solar_physical_resources():
    ...
    fig.update_layout(title="Available solar resources")
    fig.update_yaxes(
        title_text="GWh/MW/year",
        showline=True,
        linecolor=line_color,
        gridcolor=line_color,
    )
    ...
    fig2.update_layout(title="Available wind resources")
    fig2.update_yaxes(
        title_text="GWh/MW/year",
        showline=True,
        linecolor=line_color,
    ...
    fig3.update_layout(title="Technical wind generation")
    fig3.update_yaxes(
    ...
    fig4.update_layout(title="Theoretical PV generation")
    fig4.update_yaxes(
        title_text="GWh/year", showline=True, linecolor=line_color, gridcolor=line_color
    )
    fig4.update_xaxes(showline=True, linecolor=line_color)
    ...
    fig5.update_layout(title="Theoretical PV capacity")
    fig5.update_yaxes(
        title_text="GW", showline=True, linecolor=line_color, gridcolor=line_color
    )
    fig5.update_xaxes(showline=True, linecolor=line_color)
    fig5.update_layout(
        {"plot_bgcolor": "rgba(0,0,0,0)", "paper_bgcolor": "rgba(0,0,0,0)"}
    ...
    fig6.update_layout(title="Technical wind capacity")
    fig6.update_yaxes(
        title_text="GW", showline=True, linecolor=line_color, gridcolor=line_color
    )
    ...
    fig7.update_layout(title="Technical PV capacity")
    fig7.update_yaxes(
        title_text="GW", showline=True, linecolor=line_color, gridcolor=line_color
    )
    ...
    fig8.update_layout(title="Technical PV generation")
    fig8.update_yaxes(
        title_text="GWh/year", showline=True, linecolor=line_color, gridcolor=line_color
    )
...
def diesel_petrol_price(Fuel):
    ...
    fig.update_layout(  # height=350,
        font=dict(family="Calibri", size=15, color=font_color), barmode="relative"
    )
...
def per_capita_renewables():
    ...
    fig_use_cap.add_trace(
        go.Bar(
            x=countries,
            y=renewable_electricity,
            text=renewable_electricity.round(2),
            name="This research",
            marker_color="red",
        )
    )
    ...
    fig_use_cap.update_yaxes(
        title_text="Renewable electricity (MWh/year/person)",
        showline=True,
        rangemode="tozero",
        linecolor=line_color,
        gridcolor=line_color,
    ...
    fig_use_cap.update_layout(title="Renewable electricity generation per capita")
    fig_use_cap.update_traces(
        marker_line_color=font_color, marker_line_width=0, opacity=1
    )
    fig_use_cap.update_yaxes(
...
def per_capita_intensity():
    ...
    fig_use_cap.add_trace(
        go.Bar(
            x=countries,
            y=df_GDP["Demand_per_gdp"].round(1),
            text=df_GDP["Demand_per_gdp"].round(1),
            name="This research",
            marker_color="red",
        )
    )
    ...
    fig_use_cap.add_annotation(
        x=15,
        y=wolrd_average_per_capita_intensity + 0.12,
    ...
    fig_use_cap.add_annotation(
        x=15,
        y=wolrd_median_per_capita_intensity + 0.12,
    ...
    fig_use_cap.update_layout(title="Energy (primary, total) intensity  of economies")
    fig_use_cap.update_traces(
        marker_line_color=font_color, marker_line_width=0, opacity=1
    )
    fig_use_cap.update_yaxes(
...
def dependance_on_imports():
    ...
    fig_use_cap.add_trace(
        go.Bar(
            x=countries,
            y=current_share_of_imports_inc_int_transit,
            text=current_share_of_imports_inc_int_transit.round(0),
            name="This research",
            marker_color="green",
        )
    )
    ...
    fig_use_cap.update_yaxes(
        title_text="Dependance on net imports (%)",
        linecolor=line_color,
        gridcolor=line_color,
    )
    fig_use_cap.update_xaxes(
    ...
    fig_use_cap.update_yaxes(
        rangemode="tozero", linecolor=line_color, gridcolor=line_color, zeroline=False
    )
    fig_use_cap.update_xaxes(
        showline=True,
        linecolor=line_color,
...
def GDP_per_capita():
    ...
    fig_use_cap.add_trace(
        go.Bar(
            x=summary_df["Country / Territory"],
            y=summary_df["GDP Per Capita ($)"],
            text=summary_df["GDP Per Capita ($)"],
            name="This research",
            marker_color="green",
        )
    )
    ...
    fig_use_cap.add_annotation(
        x=15,
        y=wolrd_average_GDP_per_capita + 1000,
    ...
    fig_use_cap.add_annotation(
        x=15,
        y=wolrd_median_GDP_per_capita + 1000,
...
def biomass_consumption_breakdown():
    ...
    list_of_consumers = [
        "Electricity Plants",
        "Manufacturing  const. and mining",
        "Households",
        "Commerce and public services",
        "Agriculture  forestry and fishing",
        "Other consumption n.e.s",
    ]
    ...
    for country in Country_List:
        ...
        df_1.loc[:, "Biofuels and Waste"] = (
            100 * abs(df_1.loc[:, "Biofuels and Waste"]) / primary_production
        )  # /df_1.loc[:,'primary_production']
    ...
    fig.update_yaxes(
        title_text="% biofuel and waste primary production",
        showline=True,
        linecolor=line_color,
        gridcolor=line_color,
    )
    ...
    fig.update_layout(title="Breakdown of biofuel and waste consumption")
    fig.update_traces(marker_line_color=font_color, marker_line_width=0, opacity=1)
    fig.update_layout(legend={"title_text": ""})

    fig.update_yaxes(
        linecolor=line_color, gridcolor=line_color, zeroline=False, showline=True
    )
    fig.update_xaxes(
        showline=True,
        linecolor=line_color,
...
def navigation_and_int_maritime():
    ...
    for country in Country_List:
        ...
        df_1.loc[:, "Total Energy"] = (
            100 * abs(df_1.loc[:, "Total Energy"]) / total_primary_production
        )  # df_1.loc[:,'total_primary_production']
        if df_final.shape[0] == 0:
            df_final = df_1
        else:
            df_final = pd.concat([df_final, df_1])
    ...
    fig.update_yaxes(
        title_text="% of total primary production",
        showline=True,
        rangemode="tozero",
        linecolor=line_color,
        gridcolor=line_color,
