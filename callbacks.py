...
@app.callback(
    ...
    Input("wind-battery-cost", "value"),
     Input("wind-large-cost", "value"),
     Input("demand-growth", "value"),
    Input('decarb-rate', "value"),
     Input('rooftop-size', "value"),

     ]
)
def sensor_checklist(year,country,diesel_price,PV_cost,PVBatt_cost,WindBatt_cost,Wind_cost,demand_growth,decarb_rate,rooftop_size):
    ...
    figures.decarbonization_scenarios(power_generated_GWh,oil_supplied_cost, demand_growth, decarb_rate,PV_cost,PVBatt_cost,WindBatt_cost,Wind_cost)
