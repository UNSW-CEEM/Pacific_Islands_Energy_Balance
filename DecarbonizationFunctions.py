...
def calculate_community_battery_size(
    demand, residential_battery_capacity, technical_pot, total_storage_days=5
):
    ...
    community_battery = max(
        total_storage_capacity_GWh - residential_battery_capacity, 0
    )
...
def calculate_capacity_of_each_technology(
    ...
    community_battery = calculate_community_battery_size(
        demand=demand,
        residential_battery_capacity=rooftop_MW * 2 / 1000,
    ...
    return {
        ...
        "Large_PV_MW": large_PV_MW,
        "Wind_MW": wind_MW,
        "residential_battery_MWh": rooftop_MW * 2,
        "total_GWh": total_GWh,
        "Community_battery_GWh": community_battery,
...
def create_yearly_df(
    ...
    installation_df["avoided_emissions_tonne"] = installation_df["avoided_demand_GWh"] * cost_dic["emissiont/GWh_diesel"]
    installation_df["cumulative_avoided_emissions_tonne"] = installation_df[
        "avoided_emissions_tonne"
    ].cumsum(axis=0)
    installation_df["emission_cost_$"] = installation_df["cumulative_avoided_emissions_tonne"] * cost_dic["carbon_price"]



    diesel_generatio = 2.5  # kWh/Liter
    installation_df["avoided_diesel_liter"] = installation_df["avoided_demand_GWh"] / (
        diesel_generatio / 1000000
    )
    ...
    if country == "New Caledonia":
        ...
        oil = df[(df[" (to)"] == "PowerStations") & (df[" (from)"] == "Oil: Supplied")][" (weight)"].values[0]

        installation_df["avoided_diesel_liter"] = installation_df[
            "avoided_diesel_liter"
        ] * (oil / to_power_stations)

        installation_df["avoided_emissions_tonne"] = installation_df["avoided_demand_GWh"]*((oil / to_power_stations)* cost_dic[
            "emissiont/GWh_diesel"] + (coal/to_power_stations) * cost_dic["emissiont/GWh_blackCoal"])
        installation_df["cumulative_avoided_emissions_tonne"] = installation_df[
            "avoided_emissions_tonne"
        ].cumsum(axis=0)
        installation_df["emission_cost_$"] =  installation_df["cumulative_avoided_emissions_tonne"] * cost_dic["carbon_price"]
    ...
    if country == "PNG":
        ...
        # ].cumsum(axis=0)
        #
        installation_df["avoided_emissions_tonne"] = installation_df["avoided_demand_GWh"]*((perc_oil+perc_gas) * cost_dic[
            "emissiont/GWh_diesel"])
        installation_df["cumulative_avoided_emissions_tonne"] = installation_df[
            "avoided_emissions_tonne"
        ].cumsum(axis=0)
        installation_df["emission_cost_$"] =  installation_df["cumulative_avoided_emissions_tonne"] * cost_dic["carbon_price"]
    ...
    installation_df["avoided_diesel_savings"] = installation_df["avoided_diesel_savings"] + installation_df["emission_cost_$"]
...
def calculate_diesel_price(country, cost_dic):
    ...
    diesel_price = diesel_price - cost_dic["diesel_dif"]  # 20c less than retails price
...
def run_decarbonization_scenario(
    ...
    cost_dic = costs[cost_scenario] #Here I should create the cost dic after entering in the GUI
